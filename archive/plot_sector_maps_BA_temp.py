...
for dom in doms:
    ...
    # Cesta k vstupom:
    datadir = {}
    datadir['road'] = f"/data/oko/AtmostreetPostproc/{year}/Sectors"
    datadir['heat'] = f"/data/oko/AtmostreetPostproc/{year}/Sectors"
    datadir['neis'] = f"/data/oko/AtmostreetPostproc/{year}/Sectors"
    #datadir['neis'] = f"/data/users/p2993/data_cpf/netcdf/{dom}"
    #datadir['heat'] = f"/data/users/p2993/data_cpf/netcdf/{dom}"
    ...
    # Nacitanie dat:
    data = datadir[group]
    conc = xr.open_dataset(f"{data}/{dom}-{cpfa[group]}.nc")
    ...
    #concp = xr.open_dataset(f'/data/users/p2993/data_cpf/netcdf_groups/{dom}-sa-{group}.nc')
    ...
    for spc in specs:
        ...
        fstring = ""
        figtitle = f"Percentuálny podiel na celkových koncentráciách {spc} - {year} - {opis[group]}\n{fstring}"   
        levs = 0
        units = '%'
