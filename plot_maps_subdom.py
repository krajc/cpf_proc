...
def plotting (proj, C, spc, figtitle, amstab, outfile, aspect,vmin, vmax, extent):
    '''
    ...
    a = C.plot.pcolormesh( alpha = 0.4, vmin=vmin, vmax=vmax, cmap=cmap,linewidth=0, 
...
datafile['total'] = f"/data/users/p2993/data_cpf/netcdf_groups/{year}/{dom}-total-man.nc"
    # Cesta k vystupom:
pics = f"/data/users/p2993/data_cpf/pics/{year}/{dom}/conc-man"
if not os.path.exists(pics):
    os.makedirs(pics)

# Celkova koncentracia:
concT = xr.open_dataset(datafile['total'])
...
for group in groups:
    ...
    conc[group].rio.write_crs(concT.rio.crs, inplace=True)
...
for i in b.index:
    ...
    for spc in spcs:
        ...
        ctmp = concT[spc.upper()].rio.clip(subs.geometry[i])
                
        for lim in (0, limit[spc], newlimit[spc]):
            cif = concT[spc.upper()].where(concT[spc.upper()] > lim) 
            lmin = float(ctmp.min())
            lmax = float(ctmp.max())
            figname = f"{pics}/{spc}-{year}-{dom}-{i}-total-{lim}.png"
            fstring = f"(c > {lim} {unit(spc)})\n"
            #fstring = ""
            figtitle = f"Priemerné ročné koncentrácie {spc} - {year} - celkové\n{fstring}"
                    
            plotting(lcc, cif , spc, figtitle, show_st,    figname, asp,lmin, lmax, extent)
            #plotting(lcc, cif , spc, figtitle, show_st,    figname, asp, 0)

### >>>>>>>> Vykreslovanie subgroups <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        for group in groups:
            if group == 'road':
                cif = conc[group][spc.upper()]
            else:
                cif = conc[group][spc]
            ctmp = conc[group][spc].rio.clip(subs.geometry[i])
            lmin = float(ctmp.min())
            lmax = float(ctmp.max())
            figname = f"{pics}/{spc}-{year}-{dom}-{i}-{group}.png"
        #fstring = f"(c > {filt[spc]} {unit(spc)})\n"
            fstring = f"{sektor[group]}\n"
            figtitle = f"Priemerné ročné koncentrácie {spc} - {year} \n{fstring}"    
