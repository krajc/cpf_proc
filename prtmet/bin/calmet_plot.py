...
start, end, step = f"{year}-01-01 00:00:00", f"{year}-01-01 23:00:00", '3H'
rng = pd.date_range(start, end, freq=step)
...
outpth = f"/data/users/p2993/data_cpf/prtmet/{dom}"
plotdir = f"{outpth}/pics"
...
ticks = list(range(8))
ticks.append(14)
density = 2             # Vector density (1 = each gridbox plotted)
...
def plot_wind(u, v, wsp, label, figname, figtitle, norm, ticks, density):
    ...
    plt.colorbar(a,label=label, orientation="vertical", ticks=ticks, format='%.0f',shrink=0.85)
...
def plot_scalar(par, label, figname, figtitle, vmin, vmax):
    ax = plt.axes(projection=lcc)
    ax.set_extent(extent)
    ax.add_image(mapsource, 13, interpolation='bilinear')
    ax.set_title(figtitle, fontdict={'fontsize': '20', 'fontweight' : '4'})
    ...
    plt.colorbar(a,label=label, orientation="vertical", shrink=0.8)
    plt.tight_layout()
...
for time in pd.date_range(start, end, freq=step):

    figtitle = f"Time: {time}"
    
    if spc != 'wspeed':
        level = 0
        figname = f"{plotdir}/{spc}-lev{level}-{time}.png"
        plot_scalar(xds[spc].sel(times=time), units[spc], figname,figtitle, 50, 2600 )
    else:
        figname = f"{plotdir}/{spc}-lev{level}-{time}.png"
        # Selekcia terminu
        xu = xd4.u.sel(times=time, z=level)
        xv = xd4.v.sel(times=time, z=level)
        vsp = xd4.vspeed.sel(times=time, z=level)
...
ax.axis('off')
plt.savefig(figpath, dpi=300, frameon=False)
