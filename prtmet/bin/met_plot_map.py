...
for time in xds.coords['times'].values[:24]:
    ...
    if time[11:13] in tt:
        ...
        if spc != 'wspeed':
            ...
            plot_scalar(xds[spc].sel(times=time), units[spc], figname,figtitle)
        else:
            figtitle = f"Time: {time}, Level: {level}"
            figname = f"{pth}/pics/{spc}-lev{level}-{time}.png"
            # Selekcia terminu
