...
# Create met string:
metdatstring = ""
for m in metfiles[:31]:
    metdatstring = metdatstring + f'!  METDAT= {metdir}/{m} !   !END!\n'

temp = f'/users/p2993/calpuff/templates/calpuff7_{group}.inp.templ'
with open(temp) as f_obj:
    templ = f_obj.readlines()
templ = "".join(templ)

sourcedir = '/users/p2993/dbase_calpuff/source_arb'
volemdir = f'{sourcedir}/{dom}/harmo/{profile}/{ggroup}'
files = sorted(os.listdir(f'{volemdir}'))

cpfdir = f'/users/p2993/calpuff/{dom}/harmo/{profile}/{group}/{ggroup}'
outdir = f'/data/users/p2993/data_cpf/calpuff/{dom}/harmo/{profile}/{group}/{ggroup}'
lstdir = f'/work/users/p2993/calpuff/{dom}/harmo/{profile}/{group}/{ggroup}/lst'
...
text = text.replace('__nmetdat__', str(31))
