...
@author: ext33340
p2993, 7.6.2022: Skript som  trochu upravila 
"""
# citanie z *.cre file
import numpy as np
import os
...
oddelovac je | 
stlpce,ktore musi subor urcite obsahovat: 
'N','x_LCC', 'y_LCC','vyska', 'alt','priemer_mv','rychlost','teplota_mv','so2', 'nox','pm10','pm2.5','bap'
N - poradove cislo zdroja       
'''

emi_path=f"/data/users/ext33340/calpuff/sources/jelsava/jelsava_2021_opr3.txt"
...
sample_inp_calpuff="/users/p2993/calpuff/templates/calpuff7-point-annual-disc.templ"
...
#diskretne receptory
disc_file="/data/oko/krajc_atmosys/calpuff/jelsava/drec_file.dat"
#lokalita meteo suborov, v tomto adresari by mali byt len *.met subory
meteo_dir=f"/users/p2993/data/data_cpf/calmet/{domena}"

#adresar, kde sa budu ukladat output subory -  *.LST a CONC*.DAT
lstdir=f"/work/users/p2993/calpuff/{domena}/{source_type}/{group}/lst"
#adresar, kde sa budu ukladat output subory -  *.LST a CONC*.DAT
concdir=f"/data/users/p2993/data_cpf/calpuff/{domena}/{source_type}/{group}"
if not os.path.exists(lstdir):
    os.makedirs(lstdir)
...
resolution[m]
'''
dom_config=f'/users/p2993/dbase_calpuff/geodat/LCCcpf/{domena}/bbox.txt' # !!!!!!
...
config_list=[]
with open(f'{dom_config}', "r") as dom:
 config_list=dom.readlines()

origin=[]
origin=config_list[0].split()

xorigkm=float(origin[0])/1000    
yorigkm=float(origin[1])/1000
res=float(config_list[2])

#velkost mriezky
end=[]
end=config_list[1].split()
xendkm=float(end[0])/1000    
yendkm=float(end[1])/1000

# pocet bodov mriezky, musime prenasobit 1000, resolution je v metroch, suradnice v km
nx=int(1000*(xendkm-xorigkm)/res)
ny=int(1000*(yendkm-yorigkm)/res)
...
#vysledne jednotky pre emisie - kg/hr - parameter IPTU
...
#suradnice prekonvertovane na km - skontrolovat, v akych jednotkach je v zdrojovom subore s emisiami
...
# otvorime na citanie vzorovy input file a na zapis input file pre konkretny zdroj    
  ...
  for line in fin:
   ...
   #Input Group 0 
  #CALPUFF.LST subor  
   cpf_lst=f'{lstdir}/calpuff_{n}.lst'
  ...
  #meteo subory
   if "__METDAT__" in line:
     for j in range (0,pocet_meteo):
         ...
         line=f'{meteo} = {meteo_dir}/{sorted_meteo_list[j]}  !   !END! \n'
         if j<pocet_meteo-1:
           fout.write(line)
  ...
  # parametre pre definiciu domeny nacitame z konfiguracneho suboru pre domenu
   ...
   line=line.replace("__DGRIDKM__",str(res/1000))
