...
if os.path.exists(rerunfile):
    with open(rerunfile) as f_obj:
        ...
        snames = list(map(lambda x: x[-10:-5],sources))
    files = []
    for s in snames:
        files.append(f'volemarb-{ggroup}-{int(s):05d}.dat')
        
configfile = f'/users/p2993/dbase_calpuff/geodat/LCCcpf/{dom}/Domain_conf.yml'
with open(configfile) as file:
    cfg = yaml.full_load(file)
    
metdir = f'{disk}/data_cpf/calmet/{dom}'
...
outdir = f'{disk}/data_cpf/calpuff/{dom}/{group}/{ggroup}'
...
if disk != "/data/users/p2993" and not os.path.exists(f'/data/users/p2993/data_cpf/calpuff/{dom}'):
    os.symlink(f'{disk}/data_cpf/calpuff/{dom}', f'/data/users/p2993/data_cpf/calpuff/{dom}')    
