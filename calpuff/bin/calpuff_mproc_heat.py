...
def get_calpuff_elev(x,y):
    with rasterio.open("/data/oko/krajc/dbase_calpuff/geodat/elevationSK_250.tif") as rds:
...
def replace_common(template):
    ...
    text = text.replace('__nmetdat__', str(nmetdat))
...
metfiles = pd.date_range(f"{year}-01-01",f"{year}-12-31", freq="1D")
