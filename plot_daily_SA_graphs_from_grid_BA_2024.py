...
year = 2024
pozadie = 'cams' # alternativa 'rio', 'cmaq', 'cams'
...
def process_domain(dom, spc, rec, eoi, ams_daily, amsr_daily):
    ...
    elif pozadie in ['cmaq','cams']:
            datafile['backg'] = f"/data/users/p2993/data_cpf/{pozadie}/{year}/{dom}/{pozadie}-daily-backg-{dom}-{year}.csv"
    ...
    for ii in eoi.keys():
        ...
        # Denne grafy  
        if spc != 'BaP':
            figname = f"{pics}/interp_sa_daily-{ii}-{spc}{pozadie}.png"
        ...
        figname = f'{pics}/interp_sa_monthly_{ii}-{spc}{pozadie}.png'
...
for dom in doms: 
    ...
    if nrec == 0 and dom != 'juznyhont':
        ...
        writer = pd.ExcelWriter(f'/data/oko/krajc/SA/SA_{year}-{dom}-{pozadie}.xlsx')
        for spc in spcs:
