...
# -*- coding: utf-8 -*-
"""
Modifikacia calwrf_calmet_mproc.py na ucely diplomovky.
Skript spusta retaz CALWRF a CALMET. 
Predtym si pripravi  potrebne calwrf a calmet.inp subory.
Po dokonceni si posebe vycisti "medziprodukty" - m2d a calwrf.lst v /scatch. 
.m3d subory prekopiruje na /work/users/p2993 pre buduce pouzitie. 
...
def final_cleanup(dom):
    finalm3d = f'/data/users/p2993/calwrf/m3d/{dom}'
    if not os.path.exists(finalm3d):
        os.makedirs(finalm3d)
    
    if os.path.exists(f'{scratchdir}/p2993'):
        for file in os.listdir(f'{scratchdir}/p2993'):
            shutil.move(f'{scratchdir}/p2993/{file}', f'{finalm3d}')
...
# Copying out final m3d and removing scratchdir:
final_cleanup(dom)
