...
# -*- coding: utf-8 -*-
"""
Created on Apr 14 09:22:18 2022
Skript spusta pre domenu  mesiac A DEN zadane v argumente skriptu retaz CALWRF
...
if not os.path.exists(f'{wrkdir}/m3d'):
     os.makedirs(f'{wrkdir}/m3d', exit_ok=True) 
if not os.path.exists(f'{wrkdir}/lst'):
     os.makedirs(f'{wrkdir}/lst', exit_ok=True)               
if not os.path.exists(outdir):
     os.makedirs(outdir, exit_ok=True)
...
geodat = f'/users/p2993/calmet/geodat/{dom}'
...
if not os.path.exists(f'{cwrkdir}/lst/{dom}'):
     os.makedirs(f'{cwrkdir}/lst/{dom}', exit_ok=True)  
if not os.path.exists(f"{outdir}/{dom}"):
     os.makedirs(f"{outdir}/{dom}", exit_ok=True)    
if not os.path.exists(cinpdir):
     os.makedirs(cinpdir, exit_ok=True)
inpdir = f'{calwrfdir}/{dom}'
if not os.path.exists(inpdir):
    os.makedirs(inpdir, exit_ok=True)
m3ddir = f'{wrkdir}/m3d/{dom}'   
if not os.path.exists(m3ddir):
    os.makedirs(m3ddir, exit_ok=True)
if not os.path.exists(f'{wrkdir}/lst/{dom}'):
    os.makedirs(f'{wrkdir}/lst/{dom}', exit_ok=True)
