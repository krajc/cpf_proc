...
for dom in doms:
    ...
    datafile['neis'] = f"/data/oko/AtmostreetPostproc/{year}/Sectors/{dom}-{cpfa['neis']}.nc"
    datafile['heat'] = f"/data/oko/AtmostreetPostproc/{year}/Sectors/{dom}-{cpfa['heat']}.nc"
    datafile['rio'] = f"/data/oko/AtmostreetPostproc/{year}/Sectors/{dom}-Background.nc"
        # Cesta k vystupom:
    pics = f"/data/users/p2993/data_cpf/pics/{dom}_{year}/conc"
    ...
    '''
    # Nacitanie RIO:
    conc['backg'] = xr.open_dataset(datafile['rio'])
    conc['backg'] = conc['backg'].where(conc['backg'] >=0)
    conc['backg'] = conc['backg'].fillna(0.0)
    ...
    concT.to_netcdf(f"/data/users/p2993/data_cpf/netcdf_groups/{year}-{dom}-total.nc")
    # Percentualne podiely sektorov na totaloch:
    for group in groups:
        concp = conc[group]*100/concT
        concp.to_netcdf(f"/data/users/p2993/data_cpf/netcdf_groups/{year}-{dom}-sa-{group}.nc")
