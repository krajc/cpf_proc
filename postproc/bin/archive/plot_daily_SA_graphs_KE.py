...
@author: p2993
2023-04-27:
    Dost podstatna uprava skriptu aby som vylucila pouzitie manualneho dict
    stdict. Stanicne receptory sa teraz vyberaju na zaklade station_rec.yml
    pre prislusnu domenu a kontrolujusa voci dostupnym AMS pre danu latku
    Zaroven tento skript je uz osetreny na nacitavanie road RK, BB,a ZA z ineho
    adresara (specialny run pre domeny)
"""
import pandas as pd
...
ggroups = {
    'heat':['fh','nfh'],
    'neis':['annual','seasonal','fugitive','halda'],
    'road':[]
    }
...
doms = ['kosice']
nofugitive = ['martin','povazie','pohronie','hnusta']
...
def sum_cpf_group(dom, group, spc, nrec):
    ...
    if group == 'heat':
        ...
        if dom in nofugitive:
            ...
            rcdf =  rctab['annual'] + rctab['seasonal'] + rctab['fugitive'] + rctab['halda']
