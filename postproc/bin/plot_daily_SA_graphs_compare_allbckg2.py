...
# Vzhladom na moznost chybajuceho stlpca date vo vystupochatmostreet_postproc_road.py Atmostreet vytvrorim daterange
...
def process_domain(dom, spc, rec, eoi, ams_daily, amsr_daily):
    ...
    datafile['cams'] = f"/data/users/p2993/data_cpf/cams/{year}/{dom}/cams-daily-backg-{dom}-{year}.csv"
    ...
    bcams = pd.read_csv(f"{datafile['cams']}", index_col='times')
    bcams.index = pd.to_datetime(bcams.index) 
    ...
    if spc == 'BaP':
        ...
        bcams_daily = bcams[[spc]].astype(np.float64)

    for ii in eoi.keys():
        sa_daily = pd.concat([brio_daily, bcmaq_daily, bcams_daily,heat_daily[[ii]],road_daily[[ii]],neis_daily[[ii]]] , axis=1)
        ...
        cplot2 =sa_monthly[['CAMS','heating','traffic','NEIS']]  
        # Denne grafy
        ...
        # Kreslenie:
        kreslenie(ams_monthly, ii, spc, sa_daily_plt, rplot, cplot, cplot2)
       

def kreslenie(ams_monthly, ii, spc, sa_daily_plt, rplot, cplot, cplot2):
    # Kreslenie:
    ...
    colorsD = ['blue','orange','green','red','brown','black']
    ...
    ax.plot(list(ams_monthly.index), ams_monthly[ii],linestyle='None', marker='o', color='yellow', \
              markeredgecolor='black', label='AMS')
    ax.legend(ncol=5, fontsize=9, loc='lower center',bbox_to_anchor=(0.5,0.99))
    
    ax.set_xticklabels(list(rplot.index))
    ax.set_ylabel(unit(spc))
